...
const render = () => {
  ...
  document.querySelector('#app').innerHTML = `
    ...
    <main id="top">
      ...
      <section id="paket" class="packages-section section-wrap"><div class="section-heading reveal"><div><p class="eyebrow">03 / PAKET JOKI</p><h2>${c.packagesTitle}</h2></div><p class="section-intro">${c.packagesIntro}</p></div><div class="package-tabs reveal" role="tablist" aria-label="Pilih daftar paket game"><button class="package-tab" type="button" role="tab" aria-selected="false" aria-controls="arknights-panel" data-package-tab="arknights">Arknights</button><button class="package-tab" type="button" role="tab" aria-selected="false" aria-controls="endfield-panel" data-package-tab="endfield">Arknights: Endfield</button><button class="package-tab" type="button" role="tab" aria-selected="false" aria-controls="request-panel" data-package-tab="request">Request</button></div><div class="package-empty reveal reveal-delay"><span>↙</span><p>${c.empty}</p></div>${Object.entries(packages).map(([id, group]) => `<div class="package-panel" id="${id}-panel" role="tabpanel" hidden><div class="package-group-heading"><span>${group.label}</span><h3>${group.game}</h3></div><div class="package-grid ${id}-packages">${group.items.map(renderPackageCard).join('')}</div></div>`).join('')}</section>
